...
def claude_terminal_output():
    ...
    try:
        ...
        if result.returncode == 0:
            return jsonify(
                {"status": "success", "output": result.stdout, "timestamp": time.time()}
            )
        else:
        ...
        return jsonify({"error": "Output capture timeout", "status": "timeout"}), 500
    except Exception as e:
...
def claude_terminal_status():
    ...
    try:
        ...
        return jsonify({"error": "Status check timeout", "status": "timeout"}), 500
    except Exception as e:
